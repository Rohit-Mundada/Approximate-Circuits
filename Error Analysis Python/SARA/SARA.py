...
def SARA_p(a, b, Cin, ApproxCON, SIZE, groupsize):
    g = []
    g.insert(0,0)
    p = []
    p.insert(0,0)
    ...
    for i in range(0, int(SIZE/groupsize)):
        ...
        O = MUX_2_1(g[(i+1)*groupsize], GG[(i+1)*groupsize], int(ApproxRCON[i]))
...
ApproxRCON = int(input("Enter code for Exact or Approximate mode.\n"))
...
for i in range(0, iterations):
    ...
    EXACT_SUM = a + b + c
    APPROX_SUM = SARA_p(a, b, c, ApproxRCON, SIZE, groupsize)
...
filename = "SIZE{0}_groupsize{1}.txt".format(SIZE, groupsize)
file = open(os.path.join(path_name, filename), "w")

# write into file : heading and parameters
file.write("SARA ERROR ANALYSIS\n\n")
