...
def RAPCLA_p(a, b, Cin, SIZE, groupsize, window):
    g = []
    g.insert(0,0)
    p = []
    p.insert(0,0)
    ...
    for i in range(0, int(SIZE/groupsize)):
        ...
        N = GC(list(GG1[i*2+1:i*2+2])+list(GG1[i*2+2:i*2+3]),list([0])+list(GP1[i*2+2:i*2+3]),2)
        GE.insert(i, N)

        O = MUX_2_1(GE[i], GG1[i*2+2], int(GG1[i*2+1]))
...
curr_dict = os.getcwd()
dir_name = "RAPCLA_Modified_ERROR_ANALYSIS"
path_name = os.path.join(curr_dict, dir_name)

if dir_name not in os.listdir():
    os.mkdir(path_name)

filename1 = "groupsize{0}_window{1}.csv".format(groupsize, window)
...
for i in range(0, len(X)):
    ...
    for i in range(0, iterations):
        ...
        a = int(random.uniform(0,2**SIZE-1))
        b = int(random.uniform(0, 2 ** SIZE - 1))
        c = random.randint(0,1)

        # calculate exact and approximate sum
        EXACT_SUM = a + b + c
        APPROX_SUM = RAPCLA_p(a, b, c, SIZE, groupsize, window)
...
#*************************************************************************
    ...
    file.write("RAPCLA_Modified ERROR ANALYSIS\n\n")
